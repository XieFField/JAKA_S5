...
运动规划、轨迹执行与柔顺按压验证工程。当前目标是先打通“无碰撞接近 -> 建立接触
-> 简单柔顺控制 -> 安全退出”，再接入真机和后续推拿手法/视觉反馈。

> 当前代码只完成非人体仿真和真机离线接口验证。未经现场风险评估、硬件保护验证和
> 人工审批，禁止用于人体接触。

## 当前能力

- PTP、LIN、CIRC 统一规划接口与请求校验；
- MoveIt 轨迹执行、取消、超时和终点误差检查；
- ros2_control 导纳控制器切换、六维力处理和保护；
- 最小任务状态机与柔顺按压状态机；
- 仿真按摩头、`massage_tool_tip`、FT 传感器和接触工装；
- JAKA 生命周期、标准 `FollowJointTrajectory` Action 和柔顺服务适配；
- 仿真/真机独立关节限制和独立启动入口。

尚未完成：真实 JAKA S5 联机运动、真机 FT 标定、人体安全参数、具体推拿手法、视觉
消息定义和在线目标跟踪。
